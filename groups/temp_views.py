...
def show_group(request, gid):   # THIS IS FOR SHOWING INDIVIDUAL GROUP
	...
	try:
		...
		for gr in grps:
		   print (gr.agenda)
		return render_to_response("groups/show_group.html",grp,req)
	except:
		#msg = "You are not a member of this group"
		return render_to_response("groups/show_group.html",)
...
def get_members(request, gid):
       ...
       return render_to_response("groups/tabs/tab3.html",grpob)
   #else :
...
def show_grp(request):						##  THIS IS FOR SHOWING ALL THE GROUPS		
    ...
    try:
		...
		return render_to_response('groups/my_groups.html',var)
